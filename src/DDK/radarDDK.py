...
class RaderChart():
    def __init__(self, fig, variables, ranges, n_ordinate_levels = 6):
        ...
        axes = [fig.add_axes([0.15,0.15,0.67,0.67],polar = True, label = "axes{}".format(i)) for i in range(len(variables))]
...
def plot_radar(df, refh, refl, use_attributes, title, namefig):
    ...
    for data, color, bg, pokemon in zip(datas, colors, colorsbg, use_pokemons):
        if bg=='#FFFFFF':
            ...
            radar.legend(bbox_to_anchor=(0., 1.14, 1.14, .102), ncol=2, mode="expand", fontsize=20, loc=3)
        else:
            radar.plot(data, color = color, label = pokemon, linewidth=2.0)
            radar.fill(data, alpha = 0.5, color = bg)
            radar.legend(bbox_to_anchor=(0., 1.13, 1.13, .102), ncol=2, mode="expand", fontsize=20, loc=3)
