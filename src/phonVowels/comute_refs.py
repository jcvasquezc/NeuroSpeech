...
def phonationVowels(audio, path_base):
    ...
    fsp=(len(F0)/t[-1])
    print(fsp)
    t2=np.arange(0, float(len(F0))/fsp, 1.0/fsp)
    print(len(t2), len(F0))
    if len(t2)>len(F0):
        t2=t2[:len(F0)]
    elif len(t2)>len(F0):
    ...
    plot_radar(dataradar, np.asarray([1.20, 6.30, 7.70, 0.43, 0.5, 0.5]), ['Jitter', 'Shimmer', 'APQ', 'PPQ', '%U', 'var. F0'], '', path_base+'phonation2.png')    
    return F0, mjitter, mshimmer, apq, ppq, mlogE, mcd, degreeU, F0varsemi  
...
#print sys.argv
if len(sys.argv)<4:
    print('python '+sys.argv[0]+' <flder_audio> <fileresultsf0.txt> <file_features.txt> <path_base>')
    sys.exit()
audios=sys.argv[1]
...
for j in range(len(hf)):
    audio=audios+hf[j]
    F0, mjitter, mshimmer, apq, ppq, mlogE, mcd, degreeU, varF0   = phonationVowels(audio, path_base)
