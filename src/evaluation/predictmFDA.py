...
plt.plot(np.asarray([predsvr, predsvr]), [0, 8], color='r', linewidth=2, label='Speaker')
plt.legend()
...
plt.subplot(122)
plt.hist(labels[0:50], 20, color='g', alpha=0.5, label='Patients')
plt.plot(np.asarray([predsvr2, predsvr2]), [0, 11], color='r', linewidth=2, label='Speaker')
plt.legend()
plt.xlim([0,128])
plt.xlabel('Predicted MDS-UPDRS-III score')
#plt.show()
